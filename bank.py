...
# =========================
# BANK RECONCILIATION – FULL LOGIC (restored)
# =========================
def to_str(x):
    if pd.isna(x):
        return ""
...
def detect_table_bank(df_raw, max_rows=100):
        ...
        for sheet in visible_sheets:
            try:
                ...
                if df_raw is not None and not df_raw.empty:
                    df, header_row = detect_table_bank(df_raw)
...
def export_to_excel_supplier_template(working_paper_df, match_results, summary,
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        supplier_closing = summary.get('supplier_closing', 0) if summary.get('supplier_closing') else 0
        ledger_closing = summary.get('ledger_closing', 0) if summary.get('ledger_closing') else 0
...
with tab_bank:
    ...
    if run_bank_recon:
        if not st.session_state.bank_name:
            ...
            st.stop()
        if not bank_file or not ledger_file_bank:
        ...
        with st.spinner("Processing..."):
            try:
                ...
                ledger_df, ledger_closing, _ = load_ledger_bank(ledger_file_bank)
    ...
    if st.session_state.reconciliation_done:
        ...
        for c in ['LEDGER_DATE', 'BANK_DATE']:
            wp_preview[c] = wp_preview[c].apply(lambda x: format_date_bank(x) if pd.notna(x) else '')
